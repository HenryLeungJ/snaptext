...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function fetchUsers() {
      await fetch("http://snaptextquick.vercel.app/api/getusers", {
        cache: "no-store",
        headers: {
    ...
    function onConnect() {
      ...
      socket.on("message-from-user", (message, fromUserId) => {
        setRecievedUsers((prev) => {
          return { ...prev, [fromUserId]: message };
        }); //replaces new user who sent message with icon
      });
    }
          ...
          {allUsers.map((val) => {
            if (socket.id == val.userid) {
              ...
              return (
                <Alert
                  key={val.userid}
                  sentFrom={val.username}
                  message={recievedUsers[val.userid]}
                  close={() => {
                    delete recievedUsers[val.userid];
